...
import { Auditoria, TipoAcao } from '@/modules/auditoria/entities/auditoria.entity';
...
export class ReportsService {
  ...
  // ----------------------------------------------------------------------
  // Auditoria Report - via Report Service
  // ----------------------------------------------------------------------
  ...
  async generateAuditoriaReport(
    ...
    // Montar dados para o report-service
    const rows = auditorias.map((auditoria) => ({
      id: auditoria.id,
      data: format(auditoria.createdAt, 'dd/MM/yyyy HH:mm:ss'),
      usuario: auditoria.usuarioEmail || 'Sistema',
      acao: auditoria.acao,
    ...
    const reportServiceUrl = this.configService.get(
      'REPORT_SERVICE_URL',
      'http://localhost:3002',
    );

    // Chamar report-service
    try {
      const response = await firstValueFrom(
        this.httpService.post<string>(`${reportServiceUrl}/generate/auditoria`, {
          filters: {
            startDate,
            endDate,
            acao,
      ...
      console.error('Erro ao chamar report-service:', error.message);
      if (error.code === 'ECONNREFUSED') {
        throw new Error(
          `Report service não está disponível em ${reportServiceUrl}. ` +
          'Verifique se o serviço está rodando na porta 3002.'
        );
      }
      throw error;
    }
