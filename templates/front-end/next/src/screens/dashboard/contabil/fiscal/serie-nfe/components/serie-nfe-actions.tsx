import { TSerieNfeTabs } from '@/models';
...
interface Props {
  ...
  setCurrentTab: (value: TSerieNfeTabs) => void;
}

export function SerieNfeActions({ finalizeForm, setCurrentTab }: Props) {
  ...
  useEffect(() => {
    if (formState.errors && Object.keys(formState.errors).length > 0) {
      ...
      findTabWithError<TSerieNfeTabs>({ tabSchemas, formState, setCurrentTab });
    }
  }, [formState.errors]);
      ...
      >
        <Stack direction="row" spacing={2}>
          <Button
            variant="outlined"
            color="inherit"
            onClick={() => router.push(pages.dashboard.contabil.fiscal.serieNfe.list.path)}
          >
            Voltar
          </Button>
